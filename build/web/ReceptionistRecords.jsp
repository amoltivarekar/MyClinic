...
<%
    try
    {
        ...
        pst=con.prepareStatement("select * from resptselfinfo ");
        rs=pst.executeQuery();
    }
    catch(Exception e)
    {       
    ...
    <div>
        ...
        <%while (rs.next())
        {
            ...
            <tr>
                ...
                <td style="background:rgba(65,24,27,.1);" >
                    <input type="submit" class="input" value="PRESENTY" onclick="window.location='RespPresenty.jsp?ruser=<%=rs.getString(1)%>'" style=" margin:5px 5px; background: #C9A974; color:white; font-weight:bold; padding:3px 15px; border-radius: 5px; ">
                </td>
                <td style="background:rgba(65,24,27,.1);" >
                    <input type="submit" class="input" value="DELETE" onclick="window.location='ResptDelete?ruser=<%=rs.getString(1)%>'" style=" margin:5px 5px; background: #C9A974; color:white; font-weight:bold; padding:3px 15px; border-radius: 5px; ">
                </td>
            </tr>
      <%}%>
        </table>
