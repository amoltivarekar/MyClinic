...
<%
    try
    {
        ...
        pst=con.prepareStatement("select * from finalbooking where appdate='"+currentdate+"' and doctor like '"+sur+"%'");
        rs=pst.executeQuery();
    }
    catch(Exception e)
    {       
...
<div style="padding:100px 0px 700px 0px; background: url(images/banner.jpg);background-size: cover;">
    <div>
        ...
        <table width="90%">
            <tr>
                <td style="color:white; font-weight: bold; font-family:century gothics; font-size:15px; padding:10px 20px; margin:15px; background:rgba(65,24,27,.3);" readonly>
                    PATIENT NAME
                </td>                
                <td style="color:white; font-weight: bold; font-family:century gothics; font-size:15px; padding:10px 20px; margin:15px; background:rgba(65,24,27,.3);" readonly>
                    APPOINTMENT_DATE
        ...
        <%while (rs.next())
        {
            ...
            <tr>
                ...
                <td style="background:rgba(65,24,27,.1);" >
                    <input type="submit" class="input" value="MAKE BILL" onclick="window.location='makebill.jsp?puser=<%=rs.getString(1)%>'" style=" margin:5px 5px; background: #C9A974; color:white; font-weight:bold; padding:3px 15px; border-radius: 5px; /*The color of the visited link*/
}">
                </td>
            </tr>
      <%}%>
