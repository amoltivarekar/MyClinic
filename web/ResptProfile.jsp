...
<%
    try
    {
        ...
        pst=con.prepareStatement("select * from pataintapointment where pappdate='"+currentdate+"'");
        rs=pst.executeQuery();
    }
    catch(Exception e)
    {       
    ...
    <div>
        ...
        <%while (rs.next())
        {
            ...
            <tr>
                ...
                <td style="color:white; font-weight: bold; font-family:century gothics; font-size:15px; padding:7px 20px;  margin:5px 5px;  background:rgba(65,24,27,.1);">
                    <%=rs.getString(6)%>
                </td> <td style="color:white; font-weight: bold; font-family:century gothics; font-size:17px; padding:7px 20px;  margin:5px 5px;  background:rgba(65,24,27,.1);">
                    <%=rs.getString(7)%>
                </td>
                </td> <td style="color:white; font-weight: bold; font-family:century gothics; font-size:17px; padding:7px 20px;  margin:5px 5px;  background:rgba(65,24,27,.1);">
                    <%=rs.getString(8)%>
                ...
                <td style="background:rgba(65,24,27,.1);" >
                    <input type="submit" class="input" value="BOOK" onclick="window.location='PatientAppo.jsp?puser=<%=rs.getString(1)%>'" style=" margin:5px 5px; background: #C9A974; color:white; font-weight:bold; padding:3px 15px; border-radius: 5px; /*The color of the visited link*/
}">
                </td>
            </tr>
      <%}%>
